...
foreach ($summary_data as $exec) {
    ...
    $assigned_today_stmt = $conn->prepare("
        ...
        WHERE l.assigned_to = ? AND DATE(cl.calling_date) = ?
    ");
    $assigned_today_stmt->bind_param('is', $exec['executive_id'], $date);
    $assigned_today_stmt->execute();
    $assigned_today_stmt->bind_result($assigned_today);
    $assigned_today_stmt->fetch();
    $assigned_today_stmt->close();

    if ($assigned_today > 0) {
        $exec['assigned_today'] = $assigned_today;
        $active_summary_data[] = $exec;
    }
}

if (empty($active_summary_data)) {
    die("No executives have leads assigned today. No emails sent.");
}
...
$message = "Dear Manager/Admin,<br><br>Here is the summary of executive activities for " . date("d M Y", strtotime($date)) . ":<br><br>";
...
foreach ($active_summary_data as $exec) {
